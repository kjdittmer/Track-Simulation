...
val ChrisAlbert = Athlete("Chris Albert", Wheaton, [(OneHundred, 11.1), (FourHundred, 52.10), (FifteenHundred, 5.24), (OneTenHurdles, 19.83), (highJump, 1.58), (poleVault, 3.60), (longJump, 6.19), (shotPut, 9.64), (discus, 29.04), (javelin, 31.42), (Decathalon, 4927)]);
...
val IsaiahClauson = Athlete("Isaiah Clauson", Wheaton, [(TwoHundred, 23.64), (FourHundred, 54.85), (highJump, 1.59),\
 (poleVault, 2.95), (longJump, 6.56), (shotPut, 7.83), (Heptathalon, 3892)]);

fun mkTList([], r) = []
| mkTList (Athlete(name, team, [(event, time)::restEvents])::restAthletes, race) = 
  if event = race then (time, name)::mkTList(restAthletes, race)
  else mkTList(restAthletes, race);
